class PledgesController < ApplicationController
  ...
  def create
   ...
   @pledge = Pledge.create(artist_id: @artist.id, fan_id: current_fan.id, plan: @reward.title, price: @reward.price, email: params[:Email], address1: params[:Address1], address2: params[:Address2], city: params[:City], state: params[:State], zip: params[:ZIP])
    if @pledge.save
  ...
  private
  def pledge_params
    params.require(:pledge).permit(:artist_id, :fan_id, :plan, :price, :email, :address1, :address2, :city, :state, :zip)
  end
end